...
public class HessianImgLib2
{
    ...
    public static void run( ImagePlus imp, final int nThreads ) throws IncompatibleTypeException, InterruptedException, ExecutionException
    {
        new ImageConverter( imp ).convertToGray32(); // is this necessary?
        final IntervalView< FloatType > wrapped = Views.interval( ImageJFunctions.wrapFloat(imp),
                new FinalInterval(imp.getWidth(), imp.getHeight(), imp.getStackSize() / 3));
