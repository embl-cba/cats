...
import static de.embl.cba.cats.CATS.*;
...
public class InstancesUtils {
     ...
     */
    public static InstancesAndMetadata createInstancesAndMetadataFromLabels(
            ...
            ArrayList< String > classNames )
    {
        ...
        {
            Label label = labels.get( e );

            if ( label.instanceValuesArrays !=  null && ! label.instanceValuesAreCurrentlyBeingComputed )
            {
                for ( ArrayList< double[] > instanceValuesArray : label.instanceValuesArrays )
                {
                    if ( instanceValuesArray.size() != label.points.length )
                    {
                        int a = 1;
                        //logger.warning( "Label point list size does not equal instances array size" );
                        //continue;
                    }
                    ...
                    {
                        if ( ! addedPoints.contains( label.points[ p ]  ) )
                        {
                            Instance instance = new DenseInstance( 1.0, instanceValuesArray.get( p ) );
                            instancesAndMetadata.addInstance( instance );
                            instancesAndMetadata.addMetadata( Metadata_Position_X, label.points[ p ].x );
                            instancesAndMetadata.addMetadata( Metadata_Position_Y, label.points[ p ].y );
            ...
            ArrayList< Double > classWeights )
    {
        return new Callable<InstancesAndMetadata>() {
            ...
            public InstancesAndMetadata call()
            {
                ...
                final int numClasses = CATS.getNumClasses(); // TODO: getInstancesAndMetadata from label image
